...
RETURNS TABLE(osm_id bigint, geometry geometry, class text, name text, name_en text, name_de text, tags hstore, rank int, protect_class int) AS $$
    SELECT osm_id, geometry, class, name, name_en, name_de, tags, rank, protect_class
    FROM (
    SELECT osm_id, geometry,
        COALESCE(NULLIF(boundary, ''), NULLIF(leisure, '')) AS class,
        name, name_en, name_de, tags, NULL::int as rank, protect_class
        FROM (
        -- etldoc: osm_park_polygon_gen8 -> layer_park:z6
        SELECT osm_id, geometry, name, name_en, name_de, tags, leisure, boundary, protect_class
    ...
    SELECT osm_id, geometry_point AS geometry,
        ...
        )::int AS "rank", protect_class
        FROM (
        -- etldoc: osm_park_polygon_gen8 -> layer_park:z6
        SELECT osm_id, geometry_point, name, name_en, name_de, tags, leisure, boundary, area, protect_class
        FROM osm_park_polygon_gen8
